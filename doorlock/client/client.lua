...
function OpenDoorlock()
	local door_main = RageUI.CreateMenu("", "Création doorlock")
                ...
                RageUI.List("→ Type de porte ", {"Double", "Simple", "Coulissante"}, IndexType, nil, {}, true, function(Hovered, Active, Selected, Index)
                    if (Selected) then 
                        ...
                        elseif Index == 2 then
                            ...
                            RageUI.CloseAll()
                            ExecuteCommand(Config['command_create_door'])
                            ESX.ShowNotification("~g~Porte simple")
                        elseif Index == 3 then
                            _doorType = "slider"
                            RageUI.CloseAll()
                            ExecuteCommand(Config['command_create_door'])
                            ESX.ShowNotification("~g~Porte coulissante")
                            end
                        end
                    IndexType = Index;              
                end)
                ...
                RageUI.ButtonWithStyle("→ Disctance d'ouverture", nil, {RightLabel = _distToDoor}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        if dist == nil or dist == "" then
                            ESX.ShowNotification('~r~Distance invalide')
                        else
                            _distToDoor = dist
                            RageUI.CloseAll()
                            ExecuteCommand(Config['command_create_door'])
                ...
                RageUI.ButtonWithStyle("→ Accès job/job2", nil, {}, true, function(Hovered, Active, Selected)
                    if Selected then 
                        ...
                        if Job == nil or Job == "" then
                            ESX.ShowNotification('~r~Job invalide')
                        else
                ...
                RageUI.ButtonWithStyle("→ Code pin", nil, {RightLabel = pin}, true, function(Hovered, Active, Selected)
                    if Selected then 
                        ...
                        if pin == nil or pin == "" then
                            ESX.ShowNotification('~r~Pin invalide')
                        else
                            doorPin = pin
                            RageUI.CloseAll()
                            ExecuteCommand(Config['command_create_door'])
    ...
    _CreateThread(function()
        while true do
            ...
            if IsControlJustPressed(1, 38) then
                ...
                if _found then
                    ESX.ShowNotification("~g~La porte a été supprimer avec succès")
                    break
                else
                    ESX.ShowNotification("~r~La porte n'existe pas")
                end
            end

            if IsControlJustPressed(1, 47) then
                ESX.ShowNotification("~r~Opération annulé")
                break
            end
        ...
        if not allowed then
            ...
            if pin then
                if Config['CodePin_With_Menu'] then 
                    ...
                    if pinIntr ~= pin then
                        ESX.ShowNotification("Le code PIN est ~r~INVALIDE\n ~w~veuillez réessayer")
                    else
                        pulsed = true   
                        pin = nil            
                    end
            ...
            if object then
                ESX.TriggerServerCallback('guille_doorlock:cb:hasObj', function(has)
                    if has then
                        ...
                        ESX.ShowNotification("~r~Vous avez besoin d'une carte/pass pour ouvrir cette porte.")
                    end
                end, object)
            end
...
function PinDoor()
	local door_pin = RageUI.CreateMenu("", "Porte à code PIN")
